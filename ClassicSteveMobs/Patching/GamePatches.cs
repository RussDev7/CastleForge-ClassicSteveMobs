...
{
    /// <summary>
    /// Harmony patch container for ClassicSteveMobs.
    /// </summary>
    internal static class GamePatches
    {
        ...
        /// USAGE
        /// -----
        /// CSMHotkeys.SetReloadBinding("Ctrl+Shift+F3");
        /// // ... each frame (via Harmony patch) -> if (CSMHotkeys.ReloadPressedThisFrame()) { ClassicSteveConfig.LoadApply(); ... }
        ///
        /// EXAMPLES
        /// --------
        /// "F9"                 -> F9.
        ...
        /// </summary>
        internal static class CSMHotkeys
        {
            ...
            public static void SetReloadBinding(string s)
            {
                ...
                Log($"[CSMobs] Reload hotkey set to \"{s}\".");
            }
        ...
        /// Keeps the body small; heavy lifting should be inside ClassicSteveConfig.LoadApply().
        /// </summary>
        [HarmonyPatch]
        static class Patch_Hotkey_ReloadConfig_WorldEdit
        {
            ...
            {
                if (!CSMHotkeys.ReloadPressedThisFrame()) return;
                ...
                try
                {
                    ...
                    SendFeedback($"[CSMobs] Config hot-reloaded from \"{PathShortener.ShortenForLog(ClassicSteveConfig.ConfigPath)}\".");
                }
                catch (Exception ex)
                {
                    SendFeedback($"[CSMobs] Hot-reload failed: {ex.Message}.");
                }
            }
        }
        #endregion
        ...
        #region Enemy Type Registration Patch

        /// <summary>
        /// Re-registers the test enemy after vanilla enemy types are initialized.
        /// </summary>
        [HarmonyPatch(typeof(EnemyType), nameof(EnemyType.Init))]
        private static class EnemyTypeInitPatch
        {
        ...
        #region Natural Spawn Patch

        /// <summary>
        /// Optionally converts a small percent of aboveground night spawns into classic Steve mobs.
        /// Disabled by default in the config for safer testing.
        /// </summary>
        [HarmonyPatch(typeof(EnemyType), nameof(EnemyType.GetAbovegroundEnemy))]
        private static class GetAbovegroundEnemyPatch
        {
            private static readonly Random Rng = new Random();

            private static void Postfix(ref EnemyTypeEnum __result)
            {
                if (!ClassicSteveSettings.Enabled || !ClassicSteveSettings.NaturalSpawns)
                    return;

                if (Rng.NextDouble() <= ClassicSteveSettings.NaturalSpawnChance)
                {
                    ClassicSteveRegistry.EnsureRegistered();
                    __result = ClassicSteveRegistry.ClassicSteveEnemyType;
                }
            }
        }
        #endregion

        #region Custom Draw Patch

        /// <summary>
        /// Skips vanilla skinned zombie drawing for the test enemy and draws six cuboids instead.
        ///
        /// This patches SkinnedModelEntity.Draw instead of BaseZombie.Draw because BaseZombie inherits
        /// that method without declaring its own override.
        /// </summary>
        [HarmonyPatch(typeof(SkinnedModelEntity), nameof(SkinnedModelEntity.Draw))]
        private static class SkinnedModelEntityDrawPatch
        {
            private static bool Prefix(SkinnedModelEntity __instance, GraphicsDevice device, GameTime gameTime, Matrix view, Matrix projection)
            {
                if (!ClassicSteveSettings.Enabled)
                    return true;

                BaseZombie zombie = __instance as BaseZombie;
                if (!ClassicSteveRegistry.IsClassicSteve(zombie))
                    return true;

                ClassicSteveRenderer.Draw(zombie, device, gameTime, view, projection);
                return false;
            }
        }
        #endregion
